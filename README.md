...
## Important notice
This program uses the [Google Sheets API v3](https://developers.google.com/sheets/api/v3) which is a legacy API and is unfortunately scheduled for deprecation on **08/06/2021**. I originally chose this API version because it can be used without authentication or API keys on public spreadsheets. This version of the updater is no longer supported; see the [`master` branch](https://github.com/rewindrl/updater) for the new version.
